node{
	...
	stage('Deploy to K8s') {
	    sshagent(['k8s-server']) {
            sh "scp -o StrictHostKeyChecking=no trip-service-deployment-manifest.yaml ubuntu@18.220.189.167:/home/ubuntu/"
            script{
        		try{
        			sh "ssh ubuntu@18.220.189.167 kubectl apply -f ."
        		}catch(error){
        			sh "ssh ubuntu@18.220.189.167 kubectl create -f ."
        		}
        	}
        }
    }
