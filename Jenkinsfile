node{
	...
	stage('Deploy to K8s') {
        ...
        script{
        		try{
        			sh "sh ubuntu@18.222.252.18 kubectl apply -f trip-service-deployment-manifest.yaml"
        		}catch(error){
        			sh "sh ubuntu@18.222.252.18 kubectl create -f trip-service-deployment-manifest.yaml"
        		}
        }
    }
}
