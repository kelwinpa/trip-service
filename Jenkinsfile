node{
	...
	stage('Deploy to K8s') {
	    sshagent(['k8s-server']) {
            sh "scp -o StrictHostKeyChecking=no trip-service-deployment-manifest.yaml ubuntu@18.220.87.96:/home/ubuntu/"
            script{
        		try{
        			sh "ssh ubuntu@18.220.87.96 kubectl apply -f ."
        		}catch(error){
        			sh "ssh ubuntu@18.220.87.96 kubectl create -f ."
        		}
        	}
        }
    }
